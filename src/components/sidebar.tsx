  ...
  return (
    <div>
      <button
        ...
        className="w-full flex items-center gap-1.5 py-1 px-2 text-[13px] font-mono text-[#ccc] hover:bg-[#1a1a2e] transition-colors duration-100"
      >
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-2 py-1 px-2 text-[13px] font-mono transition-colors duration-100 ${
        active ? "bg-[#1a1a2e] text-[#f5f5f5]" : "text-[#888] hover:text-[#ccc] hover:bg-[#12131e]"
      }`}
    >
      <FileIcon ext={ext} />
      <span className="truncate">{label}</span>
...
function SearchPanel() {
  ...
  return (
    <>
      <div className="px-3 py-2.5 border-b border-[#1a1a2e]">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-[#12131e] border border-[#1f1f1f] text-[13px] font-mono text-[#ccc] placeholder:text-[#444] pl-8 pr-3 py-1.5 focus:outline-none focus:border-[#6366f1] transition-colors"
          />
        </div>
      </div>
      <div className="flex-1 overflow-y-auto py-1">
        {filtered.map((item) => (
          <button
            ...
            className="w-full flex items-center gap-3 px-3 py-2 text-left hover:bg-[#12131e] transition-colors duration-100 group"
          >
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {commits.map((commit) => (
          <div key={commit.hash} className="px-3 py-2.5 hover:bg-[#12131e] transition-colors duration-100 border-b border-[#111]">
            <div className="flex items-start gap-2">
              <GitCommit size={14} className="text-[#6366f1] mt-0.5 shrink-0" />
              <div className="min-w-0">
                <p className="text-[13px] font-mono text-[#ccc] leading-snug truncate">{commit.message}</p>
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-[10px] font-mono text-[#6366f1] bg-[#6366f1]/10 px-1.5 py-0.5 rounded">{commit.hash}</span>
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-4 py-5 border-b border-[#1a1a2e]">
          <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#6366f1] to-[#60a5fa] p-[2px] mb-3">
            <div className="w-full h-full rounded-full bg-[#0a0b12] flex items-center justify-center text-[13px] font-mono font-bold text-[#6366f1]">
              AO
            </div>
          </div>
          <p className="text-[15px] font-mono font-bold text-[#f5f5f5] mb-0.5">Alden Ong</p>
          <p className="text-[12px] font-mono text-[#6366f1] mb-2">Fullstack Developer</p>
        ...
        <div className="px-3 py-2">
          ...
          {socials.map((link) => (
            <a
              ...
              className="flex items-center gap-2.5 px-2 py-2 hover:bg-[#12131e] transition-colors duration-100 group"
            >
        ...
        <div className="px-4 py-3">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-center gap-2 w-full py-2 bg-[#12131e] border border-[#1f1f1f] text-[12px] font-mono text-[#ccc] hover:border-[#333] hover:text-[#f5f5f5] transition-all duration-200"
          >
            <FileText size={13} />
            View Resume
          </a>
...
export function Sidebar() {
  ...
  return (
    <aside className="hidden lg:flex fixed left-0 top-0 bottom-0 z-40">
      {/* Activity bar */}
      <div className="w-12 bg-[#06070b] border-r border-[#1a1a2e] flex flex-col items-center py-3 gap-1">
        {activityItems.map((item, i) => (
          ...
          >
            <item.icon size={20} />
            {activeTab === i && (
              <span className="absolute left-0 top-1.5 bottom-1.5 w-[2px] bg-[#6366f1] rounded-r" />
            )}
          </button>
        ))}
      </div>
      ...
      <div className="w-52 bg-[#0a0b12] border-r border-[#1a1a2e] flex flex-col overflow-hidden">
        ...
        {/* Bottom status */}
        <div className="px-3 py-2 border-t border-[#1a1a2e] flex items-center gap-2 shrink-0">
